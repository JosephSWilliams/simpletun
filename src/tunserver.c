...
unsigned char client_buffer[1500] = {0};
...
while (1){
  if ((tunnel_eagain)||(poll(&fds[0],1,256-(poll(&fds[1],1,0)*256))>0)){
    if (poll(&fds[3],1,256)>0){
      if (tunnel_eagain<1500){
        in = read(0,&tunnel_buffer[tunnel_eagain],1500-tunnel_eagain);
        if (in<1) break;}
      else in = 0;
  ...
  if ((client_eagain)||(poll(&fds[1],1,256-(poll(&fds[0],1,0)*256))>0)){
    if (poll(&fds[2],1,256)>0){
      if (client_eagain<1500){
        in = read(3,&client_buffer[client_eagain],1500-client_eagain);
        if (in<1) break;}
      else in = 0;
